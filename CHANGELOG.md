...
- `run.sh` now bind-mounts host `~/.claude/CLAUDE.md` read-only directly at `/home/node/.claude/CLAUDE.md` (shadowing the `agent-claude` volume's copy), so the host file stays canonical and edits propagate without a rebuild or volume reset. The previous copy-in-on-startup path via `/host-claude-md/` and `post-create.sh` is removed.
